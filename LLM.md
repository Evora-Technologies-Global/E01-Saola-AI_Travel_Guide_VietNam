...
layer ships to are never compiled against. Currently **41 on the JVM, 30 on the iOS
...
| 18 | **`androidDeviceTest` cannot run on API 37.** Every instrumented test fails in `Espresso.onIdle` with `NoSuchMethodException: android.hardware.input.InputManager.getInstance` — a reflection call Espresso makes that the platform removed — before any test body executes. Not a project defect and not a regression. **The scope of this row was wrong until 04.08.2026**, and wrong in the direction that costs the most: it said "Android 15 or newer" and concluded the device leg was unrunnable, so nobody ran it. Measured that day, the whole suite is **green on a Pixel Tablet AVD at API 35 (12 of 12) and on a Galaxy A16 at API 36**; only the API 37 emulator fails. The lesson is not about Espresso — it is that *"cannot run here"* written in a deviation table is a claim with an expiry date, and re-running costs a minute. Fix properly by upgrading `androidx.compose.ui:ui-test-junit4` and its transitive `androidx.test:runner`/`espresso-core`; until then, run this suite on API ≤ 36. | `androidDeviceTest/` |
...
| 26 | **A note that fails to save says nothing at all.** `DiscoveryViewModel.SaveNote` now lowers `isSavingNote` on the throw path (row #25), so the composer is usable again — but `DiscoveryState` has no error field and `DiscoveryEffect` has no message case, so the traveller still gets no word of why their writing did not save. Every other screen that can fail reports it: journal, settings and explore each carry a `ShowMessage` effect read off the effect's own payload, per the row #15 fix. Discovery is the exception because neither of its two screens has a snackbar host — adding one is a UI change in both `mobile/feature/discovery/` and `tablet/feature/discovery/`, which is why it was left out of the fix rather than bundled into it. Fix by adding `DiscoveryEffect.ShowMessage(AppError)` and an `AppSnackbarHost` to both branches. | `feature/discovery/DiscoveryContract.kt` |
