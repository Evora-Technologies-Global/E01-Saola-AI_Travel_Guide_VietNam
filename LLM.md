...
> [`docs/large-screen-layout.md`](docs/large-screen-layout.md); for what the app is built
> out of and why each choice was made, read
> [`docs/tech-stack.md`](docs/tech-stack.md).
>
> Each module also carries its own README, which states that module's rules in one place:
> [`domain/README.md`](domain/README.md) · [`data/README.md`](data/README.md) ·
> [`shared/README.md`](shared/README.md). This file stays the map across all of them; a
> rule that applies to exactly one module belongs in that module's README, and a rule that
> crosses a module boundary belongs here.
...
features, ten Contracts, ten ViewModels — and **eighteen** arrangements of them: ten
`XScreen.kt` under `mobile/feature/`, eight `XTabletScreen.kt`/`XPane.kt` under
`tablet/feature/`. So a defect in what a screen *knows* is
...
> **Eighteen is arrangements, not files, and the two numbers differ.** `DesignTokenTest`
> prints the file count on every run — currently `mobile/feature: 12`, `tablet/feature: 9` —
> which additionally counts the licences screen and its card (no shared half at all, see §3),
> and `RecentScanList.kt`, a composable only the tablet lens draws. Read the printed number
> when checking the gate's reach, and this one when arguing about the split.
...
│   │   │   └── component/            19 files, one composable per file, as under
│   │   │                             `feature/*/component/`: PageHeader, OverlayHeader,
│   │   │                             AppSnackbar, AppAsyncImage, PermissionSheet,
│   │   │                             SovereigntyBanner, SystemBars, DashedInk, LoadingState,
│   │   │                             ErrorState, EmptyState, AccentChip, ShimmerBox, Kicker,
│   │   │                             BackChip, FillGauge, SectionHeader, SurfaceLuminance,
│   │   │                             MapSourceNote
...
│   ├── values/strings.xml            Default (en), 310 keys — the fallback for every
│   │                                 unshipped locale
│   ├── values-vi|ja|ko|zh|fr|es|th/  The other seven, same 310 keys, same placeholders.
│   │                                 No exceptions: `values-vi` was one key short until
│   │                                 07.08.2026 and the rule is enforceable in one line —
│   │                                 parse the eight files and diff the name sets
...
├── core/window/WindowClassTest.kt    The branch fork and rememberCanStackVertically
└── feature/<name>/XViewModelTest.kt  One per ViewModel. **All ten have one** since
                                      04.08.2026 — see §11 row #25 for what the eight
                                      missing ones were hiding. Ten feature ViewModels,
                                      plus `MainViewModel` above; the count is what §11
                                      row #25 changed, so re-count it rather than copy it
...
| # | Deviation | Fixed by |
|---|---|---|
| 31 | **`TERMS_OF_SERVICE_URL` was a placeholder.** `feature/settings/LegalLinks.kt` holds the two documents the About card links out to; the privacy policy pointed at a real page while the terms read `https://evora.technologies/saola/terms-of-service`, which nothing served. Not a silent failure — the row opened a browser on a page that did not answer, which is worse than a dead button because the traveller has left the app to find out. | **Fixed in `f594fed`**, the commit that also made the two AI settings build decisions. Both constants now point at the published site (`evora-technologies-global.github.io/…/privacy` and `/terms`). The containment is what made it one line rather than a search: **one** `internal const val` each, read by both branches — the same argument `ReportMail.kt` makes for its single address. Row #30 is the remaining one of this shape and is still open. |
...
`DesignTokenTest.HEADER_OWNERS` lists the **fifteen** files that must comply — the nine phone
screens that draw a header (journal, settings, collection, passport, chat, discovery,
translation, explore, licences), the four tablet arrangements that have one, and the two panes
— and states every exclusion in its KDoc; the test also fails if a name in that list stops
matching a file, so renaming a screen cannot quietly drop it from the check.
