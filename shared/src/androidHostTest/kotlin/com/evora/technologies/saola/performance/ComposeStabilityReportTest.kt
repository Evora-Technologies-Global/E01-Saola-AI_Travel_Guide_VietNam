package com.evora.technologies.saola.performance
...
class ComposeStabilityReportTest {
     ...
     *  - `navController` — same shape, remembered once in `SaolaRoot` and passed down
     *    through whichever branch shell the window size selects.
     *  - `controller` — [com.evora.technologies.saola.feature.camera.CameraController], which
     ...
     *  - `effects` / `onEffect` on [com.evora.technologies.saola.core.mvi.CollectEffects] —
    ...
    private val allowedUnstableParams: Map<String, Set<String>> = mapOf(
        ...
        "SaolaRoot" to setOf("navController"),
        "SaolaApp" to setOf("navController"),
        "SaolaTabletApp" to setOf("navController"),
        "SaolaNavHost" to setOf("navController"),
        "SaolaTabletNavHost" to setOf("navController"),
    )
    ...
    fun `screen state classes are all stable`() {
        ...
        val unstable = stateClassesThatMustBeStable.filter { suffix ->
            classes.contains("unstable class com.evora.technologies.saola.$suffix ")
        }
        val missing = stateClassesThatMustBeStable.filterNot { suffix ->
            classes.contains("class com.evora.technologies.saola.$suffix ")
        }
    ...
    private fun reportFile(name: String): File {
        val dir = System.getProperty("saola.composeReportsDir")
            ?: fail(
                "System property `saola.composeReportsDir` is not set. It is configured " +
